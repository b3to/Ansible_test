...
$computers = Get-Content C:\SCR1PT5\Builds.txt
$path      =  "C:\SCR1PT5\Builds.txt" 
## verification 
if (! (Test-Path $Path)) { 
...
$TargetOU   =  "OU=Builds,OU=Machines,DC=A2M.com"  
$countPC    = ($computers).count  
 
$Space   =  Write-Host "" 
$Sleep   =  Start-Sleep -Seconds 3 
write-host "This Script will move Computer Accounts" -Fore green 
write-host "Destination location is (Builds)     " -Fore green 
...
ForEach( $computer in $computers){ 
    ...
    Get-ADComputer $computer -server mtlsvrdcwv01a | 
